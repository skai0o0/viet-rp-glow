...
export interface CharGenAssistantProps extends React.HTMLAttributes<HTMLDivElement> {
  visible: boolean;
...
const CharGenAssistant = React.forwardRef<HTMLDivElement, CharGenAssistantProps>(
  ({ visible, onSelectSuggestion, suggestions = DEFAULT_SUGGESTIONS, className, ...props }, ref) => {
    ...
    return (
      <AnimatePresence>
        <motion.div
          ...
          className={cn("fixed bottom-20 right-4 md:bottom-6 md:right-6 z-50 w-[calc(100vw-2rem)] max-w-xs", className)}
          {...props}
        >
...
CharGenAssistant.displayName = "CharGenAssistant";
