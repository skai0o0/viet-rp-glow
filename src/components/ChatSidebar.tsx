  ...
  return (
    <>
      ...
      <AnimatePresence>
        {open && (
          <motion.aside
            initial={isMobile ? { x: "-100%", opacity: 0.5 } : { width: 0, opacity: 0 }}
            animate={isMobile ? { x: 0, opacity: 1 } : { width: 280, opacity: 1 }}
            exit={isMobile ? { x: "-100%", opacity: 0 } : { width: 0, opacity: 0 }}
            transition={{ type: "spring", damping: 30, stiffness: 300, mass: 0.8 }}
            className="fixed md:relative z-50 bg-oled-surface border-r border-gray-border flex flex-col overflow-hidden"
            style={{
              height: '100dvh',
